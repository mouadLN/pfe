...
export function EditMissionDialog({ mission, open, onOpenChange, onSuccess }) {
  ...
  // Load data and populate form when mission changes
  useEffect(() => {
    if (open && mission) {
      loadAuditeurs();
      loadElements();
  ...
  const populateForm = async () => {
    ...
    setFormData({
      ...
      store: mission.store || null,
      auditeur: mission.auditeur || null,
      elements: mission.elements || [],
    });

    // Set region and load stores if store exists
    if (mission.store?.region) {
      setSelectedRegion(mission.store.region);
      await loadStoresByRegion(mission.store.region);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.auditeur) { 
        setError("Veuillez sélectionner un auditeur."); 
        return;
    }
    if (formData.elements.length === 0) { 
        setError("Veuillez sélectionner au moins un élément d'audit."); 
        return;
    }
    ...
    try {
        const payload = {
            ...
            administrateur: mission.administrateur ? { id: mission.administrateur.id } : null, // ✅ Preserve existing admin
            statut: mission.statut || "PLANIFIEE"
        };
        const elementIds = formData.elements.map(el => el.id);
        
        await missionService.update(mission.id, payload, elementIds);
        
        onOpenChange(false);
        onSuccess?.();
    } catch (err) {
        setError(err.response?.data?.message || "Erreur lors de la modification de la mission.");
    } finally { 
        setSubmitting(false);
    }
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl rounded-2xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border border-white/20 shadow-2xl overflow-hidden mx-2 sm:mx-0">
        ...
        <button
          ...
          className="absolute top-4 left-4 z-20 p-2 rounded-lg bg-gray-100 dark:bg-gray-800 lg:hidden"
        >
          {showSidebar ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
        </button>

        <div className="flex flex-col lg:flex-row min-h-[80vh] sm:min-h-[550px]">
          
          {/* Sidebar */}
          <div className={cn(
            "fixed inset-y-0 left-0 z-10 w-64 transform transition-transform duration-300 ease-in-out lg:relative lg:transform-none lg:block lg:inset-auto",
            ...
            <div className={cn(
              ...
              <div className="flex gap-2 w-full sm:w-auto order-1 sm:order-2">
                ...
                <Button onClick={handleNext} disabled={submitting} className="flex-1 sm:flex-none gap-2">
                  {submitting ? <Spinner /> : step === 4 ? "Modifier" : "Suivant"}
